...
playerForParsingMetaData(_playerForParsingMetaData)
{
    ...
    searchField.setTextToShowWhenEmpty("Search Tracks... (press enter)", juce::Colours::white);
...
void PlaylistComponent::resized()
{
    ...
    //set columns
    library.getHeader().setColumnWidth(1, 12 * getWidth() / 20);
    library.getHeader().setColumnWidth(2, 3 * getWidth() / 20);
    library.getHeader().setColumnWidth(3, 5 * getWidth() / 20);
}
        ...
        for (const juce::File& file : chooser.getResults())
        {
            ...
            else 
            {
                juce::AlertWindow::showMessageBox(juce::AlertWindow::AlertIconType::InfoIcon,
                    ...
                    fileNameWithoutExtension + " already in the playlist",
                    "OK",
                    nullptr
                );
            }
...
void PlaylistComponent::saveLibrary()
{
    ...
    std::ofstream myLibrary("playlist.csv");
...
void PlaylistComponent::loadLibrary()
{
    ...
    std::ifstream myLibrary("playlist.csv");
    std::string filePath;
    std::string length;
